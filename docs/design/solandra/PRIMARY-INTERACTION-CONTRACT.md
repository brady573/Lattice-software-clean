...
- “Do it.” -> the exact ActionProposal if current, or a newly prepared proposal from the referenced Recommendation.
- “Did it work?” -> the exact ExecutionReceipt/Verification state.

Do not re-search merely to invent provenance for an old answer. Do not treat pronoun/reference resolution as action Authorization.
...
The UI does not require formal Decision Engine participation for ordinary advice.
...
Prepared assistance remains separate from execution:

```text
Recommendation -> ActionProposal -> Authorization -> ExecutionReceipt -> Verification
```
...
- stale Knowledge/Recommendation/ActionProposal presentation is retired where its basis is no longer valid;
...
7. requires formal Decision Engine state for every Recommendation;
8. converts Recommendation into Authorization;
9. converts ExecutionReceipt into Verification;
10. reconstructs old authority/provenance from prose when exact governed refs exist;
11. exposes internal provider/worker/Run machinery instead of useful state;
12. fabricates a formal winner/frontier collapse;
13. makes mobile/zoom/accessibility materially less usable.
