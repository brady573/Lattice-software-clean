  ...
  const contract = JSON.stringify([
    {
      ...
      recommendation: "exact verbatim USER-supplied option excerpt",
      basis: [{ knowledgeId: "one supplied Knowledge ID", claimIds: ["supplied claim IDs from that Knowledge"] }],
      rationale: ["advisory judgment only; never a new external factual premise"],
      tradeoffs: ["advisory tradeoff only; never a new external factual premise"],
      ...
      alternatives: ["exact verbatim USER-supplied option excerpt"],
    },
    { status: "NEEDS_KNOWLEDGE", knowledgeNeeds: ["external fact needed"], reason: "why it is required" },
    { status: "NEEDS_CLARIFICATION", question: "material USER clarification or USER-supplied option needed", reason: "why it changes the advice" },
    ...
    messages: [
      {
        ...
        content: [
          ...
          "For RECOMMENDATION, recommendation and alternatives are selections, not generated factual prose: copy each selected option exactly and verbatim from authoritative USER objective/current USER context. Do not paraphrase, embellish, or append a reason to option text. If no USER-supplied option can be selected exactly, return NEEDS_CLARIFICATION instead of inventing a durable option.",
          ...
          "Every external factual basis reference must use only supplied Knowledge IDs and claim IDs. Lattice renders the factual support later from those exact governed claims; do not rely on rationale, tradeoffs, assumptions, uncertainties, recommendation, or alternatives to establish factual support.",
          "If no external factual premise is needed, a Recommendation may use an empty Knowledge basis and reason only from authoritative USER intent/current USER context. If an external fact is required but not supplied, return NEEDS_KNOWLEDGE instead of inventing it.",
          ...
          "rationale and tradeoffs are transient drafting/advisory judgment only. They may not introduce external factual propositions and are not durable Recommendation support.",
    ...
    messages: [
      {
        ...
        content: [
          ...
          "This audit is only a drafting guardrail. GROUNDED does not grant factual authority or durable status; Lattice independently constructs durable Recommendation content from exact USER material and governed Knowledge.",
