...
/**
 * `proposal` remains present on both result modes only as a temporary source
 * compatibility surface for existing governed consumers. In CONVERSATION mode
 * it is generated by Lattice, carries no inferred USER meaning, and must not be
 * used for routing or authority. The model is not asked to populate it.
 */
...
function conversationCompatibilityProposal(): SolandraSemanticProposal {
  return {
  ...
  async interpret(input: SolandraCognitionInput): Promise<SolandraCognitionResult> {
    ...
    if (parsed.mode === "CONVERSATION") {
      return Object.freeze({
        ...
        proposal: conversationCompatibilityProposal(),
