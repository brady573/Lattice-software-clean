    ...
    messages: [
      {
        ...
        content: [
          ...
          "Use CONVERSATION for ordinary discussion, explanation, brainstorming, transformation of USER-provided material or prior non-governed conversational material, hypothetical reasoning, or other non-consequential conversation that does not materially require a Lattice trust boundary. Do not use CONVERSATION to transform prior Solandra factual content when supplied governed Knowledge is the factual basis of that content; that is historical Knowledge reference help.",
          ...
          "Historical Knowledge references may be implicit through ordinary conversational coreference; the USER does not need to name Knowledge or its ID. When the USER asks to explain, simplify, restate, or otherwise transform prior Solandra factual content whose basis is supplied historical Knowledge, use the matching EXPLAIN_REFERENCE or SIMPLIFY_REFERENCE mode and set referencedKnowledgeId to the exact supplied Knowledge ID. When the USER asks for that prior state\'s sources, support, or provenance, use SOURCES_REFERENCE with the exact supplied Knowledge ID. If the supplied Knowledge has no admitted source, still use SOURCES_REFERENCE so Lattice can faithfully report that absence. Never invent a Knowledge ID.",
          ...
          "For a new KNOWLEDGE or FRESH_RESEARCH Run, set knowledgePresentation to SOURCES when the USER wants the resulting governed Knowledge presented primarily as the source list; otherwise use ANSWER. Infer this naturally from the request rather than from keywords. This is non-authoritative presentation form only: it must never strengthen or weaken factual authority, evidence admission, uncertainty, provenance, source suitability, or any trust requirement. Historical source follow-ups remain SOURCES_REFERENCE with an exact supplied Knowledge ID.",
