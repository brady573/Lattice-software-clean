    ...
    messages: [
      {
        ...
        content: [
          ...
          "Classify objectiveRelation by comparing the current USER message with the current canonical objective, not merely by noticing that both occur in the same Conversation.",
          "Use NEW_OBJECTIVE when the current USER message expresses a self-contained goal, question, planning task, or requested outcome that is materially different from the current canonical objective. A clear change of subject or task remains NEW_OBJECTIVE even when prior messages are available as conversational history.",
          "Use CONTINUE only when the current USER message materially depends on, elaborates, narrows, explains, requests sources for, or otherwise continues the current canonical objective or an addressable governed object from it. Short referential follow-ups such as 'Why?', 'Explain that more simply.', or 'What are your sources?' normally CONTINUE when their referent is clear.",
          "Use CORRECTION when the USER is revising, correcting, or replacing meaning within the current objective rather than starting an independent topic. Do not use CORRECTION merely because a new objective supersedes the old one in time.",
          "For an unambiguous NEW_OBJECTIVE directly stated by the USER, proposedObjective should reproduce the current USER message exactly. For CONTINUE, proposedObjective should normally be null. For an explicit direct correction whose complete replacement objective is the current USER message, reproduce that exact message; otherwise propose the corrected objective and surface confirmation through materialAmbiguity when needed.",
          "If it is genuinely unclear whether a turn continues the current objective or starts a materially different objective, and that distinction could materially change Knowledge, Recommendation, decision support, or later consequential work, set materialAmbiguity with one minimal clarification question instead of silently guessing.",
